...
import "./SafeMath.sol";
import "./SleepyswapLibrary.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library SleepyswapLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = SleepyswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        require(
            ...
            "SleepyswapArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        if (aToB) {
            uint256 amountOut = SleepyswapLibrary.getAmountOut(
                amountIn,
                reserveA,
                reserveB
            );
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = SleepyswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ISleepyswapPair pair = ISleepyswapPair(
            SleepyswapLibrary.pairFor(factory, tokenA, tokenB)
        );
        bool feeOn = ISleepyswapFactory(factory).feeTo() != address(0);
